...
<!-- Start Css -->
<link rel="stylesheet" type="text/css" href="css/style.css" /> <!-- font style included here -->
...
if(isset($_POST['submit'])) {  //email user //
	...
	$sql = mysql_query("SELECT id,email,username FROM student WHERE email='$email'") or die (mysql_error());
...
$db->query_update('student', $update, "email='$email'"); //update with password expiry time
$db->query_update('users', $update, "email='$email'"); //update with password expiry time



// start comment out
$mailer = "<div style='margin: 0 auto; background-color:f0e1ae; width:520px; padding:12px;'><img src='http://studentbook.itmind4u.com/images/student_book_logo.png'><br><br>Hi ".$row->username.",<br><br> There was recently a request to change the password on your account. If you requested this password change, please set a new password by following the link below:<br><br>
...
<form id="form" method="post" action="#"> 
    ...
    <h2>Enter your email to reset your password. You may need to unblock studentbook2013@studentbook.com or check your spam folder.</h2>
...
<?php } else {
	$sql = mysql_query("SELECT id,email,username,reset,expire FROM student WHERE reset='$get_code'") or die (mysql_error());
	...
	elseif (time() - $time > 86400 /* valid for 24 hours */) {
		...
		$update['expire'] = '';
		$update['reset'] = '';
		$db->query_update('student', $update, "reset='$reset'");
		$db->query_update('users', $update, "reset='$reset'");
...
if(isset($_POST['submit'])) {
		...
		if(trim($apassword) == '') {
			...
			$update['expire'] = '';
			$update['reset'] = '';
			$db->query_update("student", $update, "reset='$get_code'");
			$db->query_update("users", $update, "reset='$get_code'");
...
?>
<div class="clear"></div>
  <h2><a href="../user/index.php">User Login</a></h2>
</body>
</html>